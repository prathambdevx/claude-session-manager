...
export function startClearReconciliationPoller(intervalMs = 1500) {
  if (reconcilePollerStarted) return; // idempotent — tests may import routes.ts more than once
...
export async function handleRequest(req: Request): Promise<Response> {
  ...
  if (url.pathname === "/api/sessions" && req.method === "GET") {
    const [sessions, running, reconciledMeta, tickets, agents, delegations, todos, board, todoBoard, projectBoards] = await Promise.all([
      scanAllSessions(),
      loadRunning(),
      reconcileNow(),
      loadTickets(),
